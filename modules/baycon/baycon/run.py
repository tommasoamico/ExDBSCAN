...
import modules.baycon.baycon.bayesian_generator as baycon
import modules.baycon.baycon.time_measurement as time_measurement
from modules.baycon.common.DataAnalyzer import *
from modules.baycon.common.Target import Target
from ucimlrepo import fetch_ucirepo
from typing import Optional
from sklearn.model_selection import train_test_split
from modules.constants import dataPath, modelsPath
...
def prepare_model_and_data(dataset, model_name, target:Target, 
    ...
    X:pd.DataFrame = dataframe.drop([target.target_feature()], axis=1).values
...
def execute(dataset_name, target:Target, initial_instance_index, categorical_features=[], actionable_features=[]):
    total_runs = 1
    ...
    for model_name in models_to_run:        
        ...
        for run in range(total_runs):
            ...
            model, X, Y, feature_names = prepare_model_and_data(
...
def prepareDataCustom(dataset, target:Target, 
                           categorical_features, uci:Optional[int] = None, modelName:str = 'RF'):
    
    if uci:

        dasasetUci = fetch_ucirepo(id=uci)

        X:pd.DataFrame = dasasetUci.data.features         

        Y:pd.Series = dasasetUci.data.targets.iloc[:,0] 

        Y:pd.Series = Y[~np.any(np.isnan(X), axis=1)]

        X:pd.DataFrame = X[~np.any(np.isnan(X), axis=1)]        
    ...
    if modelName == "RF":
        ...
        model:MLPClassifier = MLPClassifier(**{'hidden_layer_sizes':(15, 15), 'batch_size':16, 'max_iter':50, 'random_state':1})    
    
    # normalize data
    xTrain, xTest = scaleTrainTest(xTrain=xTrain, xTest=xTest)
    ...
    try:
        ...
        print("Finished training")
        pickle.dump(model, open(modelsPath / model_filename, 'wb'))

    
    return model, xTrain, xTest, yTrain, yTest, feature_names


def executeCustom(target:Target, initial_instance_index, model, X, Y, featureNames, categorical_features=[], actionable_features=[]):
    #print(X)
    #if len(categorical_features) > 0:
     #   print('YES')
     #   X=encode(X, categorical_features=categorical_features)
    ...
    for run in range(total_runs):
        ...
        data_analyzer = DataAnalyzer(X, Y, featureNames, target, categorical_features, actionable_features)
        X, Y = data_analyzer.data()
        X = np.asarray(X)
        Y = np.asarray(Y)
        initial_instance = X[initial_instance_index]
        initial_prediction = Y[initial_instance_index]
        #print(f"--- Executing: {dataset_name} Initial Instance: {initial_instance_index} Target: {target.target_value_as_string()} Run: {run} ---")
