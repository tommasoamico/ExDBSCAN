...
def encode(X, categorical_features):
    from modules.baycon.common.MultiColumnLabelEncoder import MultiColumnLabelEncoder
...
def scaleTrainTest(xTrain, xTest):
    ...
    scalerInstance:MinMaxScaler = MinMaxScaler()

    scaledxTrain = scalerInstance.fit_transform(xTrain)

    scaledxTest = scalerInstance.transform(xTest)
...
class DataAnalyzer:
    def __init__(self, X, Y, feature_names, target, cat_features=None, actionable_features=None, feature_weights=None):
        ...
        #print('NUMERICAL', self._numerical_features)
