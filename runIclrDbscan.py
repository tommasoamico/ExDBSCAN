...
#toyDataset:pd.DataFrame = pd.read_csv(toyDatasetPath)

#x = toyDataset[['x', 'y']].to_numpy()
...
for datasetName in tqdm(datasets):
    ...
    for i, (indexPoint, targetClusters) in enumerate(tqdm(pointsToTest.items())):
        ...
        for targetCluster in targetClusters:
            ...
            if targetCluster != -1:
                ...
                #newGraphExDbscan, newNodeIndicesExDbscan, _ = addPointsToGraphExdbscan(
                 #   G=graph, newPoints=allCounterfactuals, targetCluster=targetCluster, greedySet=greedySetExDbscan
                #)
                

                

                #print(len(allCounterfactuals), len(validCounterfactualsBaycon))
                ...
                #print('DIVERSITY', diversityExDbscan, diversityBaycon)

                #print('PROXIMITY', proximityDbscan, proximityBaycon)
    ...
    resultsDf.to_csv('data/iclrDbscan/resultsDiversityNonActionableDPP.csv', index=False)
        ...
        #allResultsDataset[i,:] = [indexPoint, targetCluster, proximity]
